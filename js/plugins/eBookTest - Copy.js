...
Scene_Database.prototype.create = function() {
    ...
    var listRect = new Rectangle(0, 0, listWidth, listHeight);
    this._listWindow = new Window_DatabaseList(listRect);
    this._listWindow.setHandler('ok', this.onListOk.bind(this));
    this._listWindow.setHandler('cancel', this.popScene.bind(this));
    this.addWindow(this._listWindow);

    var databaseRect = new Rectangle(listWidth, 0, Graphics.boxWidth - listWidth, Graphics.boxHeight);
    this._databaseWindow = new Window_Database(databaseRect);
    this._databaseWindow.setHandler('cancel', this.onDatabaseCancel.bind(this));
...
Window_DatabaseList.prototype.isCursorMovable = function() {
    return this.isOpen() && this.active;
};

Window_DatabaseList.prototype.cursorDown = function(wrap) {
    var index = this.index();
    var maxItems = this.maxItems();
    var maxCols = this.maxCols();
    if (index < maxItems - 1 || (wrap && maxCols === 1)) {
        index = (index + maxCols) % maxItems;
    } else if (wrap) {
        index = (index % maxCols) + maxCols;
    }
    this.select(index);
};

Window_DatabaseList.prototype.cursorUp = function(wrap) {
    var index = this.index();
    var maxItems = this.maxItems();
    var maxCols = this.maxCols();
    if (index >= maxCols || (wrap && maxCols === 1)) {
        ...
        index = (maxItems - 1 - index % maxCols) * maxCols + index % maxCols;
    }
    this.select(index);
};
...
Window_Database.prototype.update = function() {
...
Game_System.prototype.addDatabaseEntry = function(entryType, entryId, entryData) {
    ...
    // Check if entryData is already an object
    if (typeof entryData === 'string') {
        ...
        databaseData[entryType][entryId] = entryData;
    }
};
